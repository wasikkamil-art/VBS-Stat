...
// ── TOLL PROXY — PTV Developer Routing API (realne myto per kraj, oficjalne stawki UE) ──
//    Waypoints trasy + profil pojazdu → myto per kraj w EUR (PTV sam konwertuje waluty).
//    Klucz z config/toll.ptvKey (admin). Europejski dostawca, liczy TEŻ Polskę; darmowy
//    plan 500 wywołań/dzień. Kalkulator używa jako główne źródło; flat-rate = fallback.
...
exports.setTollKey = onCall(
  ...
  async (request) => {
    ...
    await db.doc("config/toll").set({
      ptvKey: apiKey.trim(),      // klucz PTV Developer
      provider: "ptv",
